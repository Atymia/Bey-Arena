...
function aiSteer(ai, player, dt) {
  ...
  // Scale by spin like the player's steering, so a dying AI bey slows too.
  const spinScale = 0.4 + ai.spin * 0.6;
  ai.vx += fx * spinScale * dt;
  ai.vy += fy * spinScale * dt;
}
...
function updateBattle(dt) {
  ...
  if (!inGrace) {
    ...
    // Steer force scales with current spin and the bey's speed stat, like the
    // reference: a fresh, fast bey responds crisply; a dying one gets sluggish.
    const steerForce = 520 * player.spin * (0.5 + player.bey.stats.speed / 20);
  ...
  [a, b].forEach((f) => {
    ...
    const maxSpeed = 80 + f.bey.stats.speed * 12;
    const sp = Math.hypot(f.vx, f.vy);
    if (sp > maxSpeed) { f.vx = (f.vx / sp) * maxSpeed; f.vy = (f.vy / sp) * maxSpeed; }
    // Exponential damping (frame-rate independent), like cannon-es LINEAR_DAMPING.
    // Lighter damping = more glide/momentum = the fluid feel of the reference.
    const damping = 0.9;
    const damp = Math.pow(1 - damping * 0.22, dt * 60);
    f.vx *= damp;
    f.vy *= damp;
    f.x += f.vx * dt;
    f.y += f.vy * dt;
...
// toward the opponent). It just adds a burst of speed + a damage boost for its short
// duration; whether it hits depends entirely on how the player has positioned and
// aimed their bey. If they ram the opponent, the normal collision (boosted) does the
// damage. The opponent can dodge by moving out of the way in time.
function triggerBaseMove(attacker, defender) {
  if (attacker.baseTimer > 0) return; // not ready
  // Thrust along current heading; if nearly still, give a gentle forward nudge using
  // the bey's spin facing so the move always does something.
  let hx = attacker.vx, hy = attacker.vy;
  let mag = Math.hypot(hx, hy);
  if (mag < 1) { hx = Math.cos(attacker.spinYaw); hy = Math.sin(attacker.spinYaw); mag = 1; }
  hx /= mag; hy /= mag;
  attacker.vx += hx * 200;
  attacker.vy += hy * 200;
...
function onTilt(e) {
  ...
  const dx = sx - tiltBase.x;
  const dy = sy - tiltBase.y;
  tiltVector.x = clamp(dx / 28, -1, 1);
  tiltVector.y = clamp(dy / 28, -1, 1);
}
function enableTilt() {
  function attach() {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('orientationchange', () => { if (tiltEnabled) tiltRecalibrate = true; });
