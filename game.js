...
function onTilt(e) {
  ...
  // Map the device tilt to screen axes. In landscape the phone is rotated 90°, so the
  // device beta (front/back lean) drives one screen axis and gamma (left/right lean)
  // the other, with signs depending on which way it was rotated.
  // Convention: tilt the TOP of the phone away from you -> bey moves UP (toward screen
  // top); tilt left -> bey moves left. This matches how you'd nudge a real dish.
  const angle = (screen.orientation && screen.orientation.angle);
  const a = (angle == null ? (window.orientation || 0) : angle);
  let sx, sy; // raw tilt mapped to screen right(+x) and screen up(+y)
  if (a === 90) {
    // home button / notch on the right
    sx = -e.gamma;   // lean right -> +x
    sy = -e.beta;    // lean top away -> +y (up)
  } else if (a === -90 || a === 270) {
    // home button / notch on the left
    sx = e.gamma;
    sy = e.beta;
  } else if (a === 180) {
    sx = -e.gamma; sy = e.beta;
  } else {
    // portrait
    sx = e.gamma; sy = -e.beta;
  }
  ...
  if (tiltRecalibrate || tiltBase.beta === null) {
    ...
    tiltSmooth.x = 0; tiltSmooth.y = 0;
    return;
  }
  let dx = sx - tiltBase.x;
  let dy = sy - tiltBase.y;
  // Small dead zone so resting the phone doesn't drift the bey.
  const DEAD = 2.5; // degrees
  dx = Math.abs(dx) < DEAD ? 0 : dx - Math.sign(dx) * DEAD;
  dy = Math.abs(dy) < DEAD ? 0 : dy - Math.sign(dy) * DEAD;
  // Sensitivity: ~40° tilt reaches full steer. Note +y (screen up) must map to a
  // NEGATIVE world-Y move because the arena's +y goes downward on screen.
  const tx = clamp(dx / 40, -1, 1);
  const ty = clamp(-dy / 40, -1, 1); // invert so "up tilt" => up on screen
  // Smooth toward the target each reading so motion is fluid, not jittery.
  tiltSmooth.x += (tx - tiltSmooth.x) * 0.25;
  tiltSmooth.y += (ty - tiltSmooth.y) * 0.25;
  tiltVector.x = tiltSmooth.x;
  tiltVector.y = tiltSmooth.y;
}
function requestFullscreen() {
  const el = document.documentElement;
  const fn = el.requestFullscreen || el.webkitRequestFullscreen || el.mozRequestFullScreen || el.msRequestFullscreen;
  if (fn) { try { fn.call(el).catch(() => {}); } catch (e) { /* ignore */ } }
}

function enableTilt() {
  requestFullscreen(); // immersive: hide the browser bar and system UI (user-gesture)
  function attach() {
